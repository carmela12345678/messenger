...
const UpdateGroupChatModal = ({ fetchMessages, fetchAgain, setFetchAgain }) => {
  ...
  const handleAddUser = async (user1) => {
    ...
    if (selectedChat.groupAdmin._id !== user._id) {
      toast({
        title: "Only the creator can add a user",
        status: "error",
        duration: 5000,
        isClosable: true,
        position: "bottom",
  ...
  const handleRemove = async (user1) => {
    if (selectedChat.groupAdmin._id !== user._id && user1._id !== user._id) {
      toast({
        title: "Only the creator can remove someone!",
        status: "error",
        duration: 5000,
        isClosable: true,
        position: "bottom",
  ...
  return (
    <>
      ...
      <Stack direction='row'>
       ...
       <a href="javascript:void(0);" class="btn btn-outline-secondary" onClick={onOpen}><i class="fa fa-cogs"></i></a>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            ...
            <Button onClick={() => handleRemove(user)} colorScheme="red"  >
              <Text m={1}>Leave Group   </Text>
               {/* <ArrowForwardIcon m={1}/>  */}
